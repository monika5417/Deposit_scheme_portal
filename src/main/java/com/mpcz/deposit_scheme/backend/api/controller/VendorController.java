...
@Api(value = "VendorController", description = "Rest api for Vendor Controller ")
@RestController
@RequestMapping(RestApiUrl.VENDOR_API)
public class VendorController {
	...
	@ApiOperation(value = "Retrieve  Consumer Applications By Vendor Id", notes = "Pass Vendor id", response = Response.class, responseContainer = "List", tags = RestApiUrl.VENDOR_TAGS)
	@ApiResponses(value = {
	...
	@GetMapping(RestApiUrl.GET_ALL_CONSUMERS_APPLICATION_BY_VENDOR_ID)
	public ResponseEntity<Response<?>> AllConsumerApplicationByVendorId(@PathVariable long id,
			HttpServletResponse httpServletResponse)
			throws FormValidationException, InvalidAuthenticationException,DataNotFoundException ,ConsumerNotFoundByVendorIdException{

		final String method = RestApiUrl.VENDOR_API + RestApiUrl.GET_ALL_CONSUMERS_APPLICATION_BY_VENDOR_ID
				+ " : retrieveAllVendorSelectedConsumerApplications()";
		LOG.info(method);
		Response<ConsumerApplicationDetail> response = new Response<ConsumerApplicationDetail>();
		List<ConsumerApplicationDetail> listOfConsumerApplicationDetail= vendorService.findAllConsumerApplicationByVendorId(id);
		response.setList(listOfConsumerApplicationDetail);
	...
	@ApiOperation(value = "Save Consumer Accepted Or Rejected Details By Vendor", notes = "Pass ConsumerAppId,RejectedRemark,IsRejected", response = Response.class, responseContainer = "List", tags = RestApiUrl.VENDOR_TAGS)
	@ApiResponses(value = {
	...
	@ApiResponse(code = ApiResponseCode.FORBIDDEN_CODE, message = ResponseMessage.FORBIDDEN),
	@ApiResponse(code = ApiResponseCode.NOT_FOUND_CODE, message = ResponseMessage.NOT_FOUND) })
	@PostMapping(RestApiUrl.ADD_CONSUMER_APPLICATION_DETAILS)
	public ResponseEntity<Response<?>> addConsumerApplicationAcceptanceOrRejectionDetail(@RequestBody @Valid VendorRejectionForm vendorRejectionForm,
	BindingResult bindingResult, HttpServletResponse httpServletResponse)
	throws FormValidationException, InvalidAuthenticationException, ConsumerNotFoundByApplicationIdException, ConsumerApplicationDetailException {

	final String method = RestApiUrl.VENDOR_API + RestApiUrl.ADD_CONSUMER_APPLICATION_DETAILS + " :  addConsumerApplicationAcceptanceOrRejectionDetail() ";
	LOG.info(method);
	...
	ConsumerApplicationDetail updateConsumerApplicationDetailsByApplicationId = vendorService.updateConsumerApplicationDetailsByApplicationId(vendorRejectionForm);
	   
	response.setList(Arrays.asList(updateConsumerApplicationDetailsByApplicationId));
