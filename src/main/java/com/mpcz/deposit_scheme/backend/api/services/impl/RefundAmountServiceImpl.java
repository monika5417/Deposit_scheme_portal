	...
	public RefundAmount saveRefundDetails(RefundAmount refundAmount) throws ConsumerApplicationDetailException {
		...
		if (Objects.isNull(consumerAppDetail)) {
			...
			if (save != null) {
				...
				consumerAppDetail.setErpVersionInApi(consumerAppDetail.getErpVersionInApi() + 1);
...
//							sspRegCharge ka koi bhi amount return nhii krna hai confirmed by ankit sir  23-02-2026
							refundableAmount = applicationNoAndTxnAmountDB.getTxnAmount().subtract(totalCgstSgst)
									.subtract(sspRegCharge);
						} else {
							refundableAmount = applicationNoAndTxnAmountDB.getTxnAmount().subtract(totalCgstSgst);
						}
						...
						if (sspRegCharge != null) {
							refundableAmount = applicationNoAndTxnAmountDB.getTxnAmount().subtract(sspRegCharge);
						} else {
							refundableAmount = applicationNoAndTxnAmountDB.getTxnAmount();
						}
					}
...
//					added this given check on 21-08-2025 Monika Rajpoot 
					...
					BigDecimal Tds2_194J = erpEstimateDB.getU_sec_194J_tds2() != null
							? erpEstimateDB.getU_sec_194J_tds2()
							: BigDecimal.ZERO;
					BigDecimal Tds2_194c = erpEstimateDB.getU_sec_194C_tds2() != null
							? erpEstimateDB.getU_sec_194C_tds2()
							: BigDecimal.ZERO;
					totalHeadAmount = supervisionAmount.add(depositAmount).add(securityDepositAmount).add(kvaAmount)
							.add(colonyOrSlumAmount).add(returnAmount).add(sspMeterCost).subtract(Tds2_194J)
							.subtract(Tds2_194c);
...
//											this case is added for govt. application and NOW 12 Date: 23-12-2025
											if (erpData.getRegistrationCharges() != null && erpData
													.getRegistrationCharges().equals(BigDecimal.valueOf(1180))) {
												refundableAmount = refundableAmount.subtract(BigDecimal.valueOf(1180));
											}
											totalWapaskrneWalaPaisa = totalWapaskrneWalaPaisa.add(refundableAmount);
											...
											if (Objects.nonNull(sspRegCharge)) {
												sspRegCharge = sspRegCharge.subtract(BigDecimal.valueOf(5));
											}
...
//										sspRegCharge ka koi bhi amount return nhii krna hai confirmed by ankit sir  23-02-2026
									...
									BigDecimal billdeskAmount = new BigDecimal(bill.getAmount());
									if (sspRegCharge != null) {
										refundableAmount = billdeskAmount.subtract(sspRegCharge);
									} else {
										refundableAmount = billdeskAmount;
									}
