...
import com.mpcz.deposit_scheme.backend.api.dto.CheckBoxHtDTO;
...
@Api(value = "HtController", description = "Rest api for HTController .")
@RestController
@RequestMapping(RestApiUrl.HT_API)
public class HtController {
	...
	@ApiOperation(value = "save ht check box details", notes = "save ht check box details",response = Response.class,responseContainer = "List", tags = RestApiUrl.HT_API )
	@ApiResponses(value = {
	...
	public ResponseEntity<Response<?>> save(@RequestBody CheckBoxHtDTO boxHtDTO,
		...
		Response<CheckBoxHt> response =new Response();
		...
		CheckBoxHt ht =new CheckBoxHt();
		
		ModelMapper convetHtDtoToHtDemoain = new ModelMapper();
		convetHtDtoToHtDemoain.map(boxHtDTO, ht);
		
		CheckBoxHt htdb=	htService.save(ht);
		...
		if(htdb!=null) {
			...
			response.setList(Arrays.asList(htdb));
	...
	public ResponseEntity<Response<?>> retrieveDtr(@PathVariable long id, HttpServletResponse httpServletResponse)
		 ...
		 LOG.info(method);
		 CheckBoxHt dtrdb=	htService.getHtDetails(id);
		 Response<CheckBoxHt> response =new Response<CheckBoxHt>();
		 response = new Response<>();
			response.setMessage(ResponseMessage.SUCCESS);
			response.setCode(ResponseCode.OK);
